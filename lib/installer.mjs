...
// fast path.
//
// Invokes Playwright's CLI directly via require.resolve rather than
// shelling out to `npx playwright install`. Two reasons:
//   1. npx emits a misleading "running outside project context"
//      warning — Playwright thinks it's being invoked without a host
//      project even though it IS a direct dep of sessions-dashboard.
//   2. Using `spawn(...)` with shell:true and args triggers Node's
//      DEP0190 deprecation warning. Since we're invoking `node` with
//      a known-absolute path, shell:false is fine and safer.
export function prefetchChromium(log = console.log) {
  return new Promise((resolve, reject) => {
    let playwrightCli;
    try {
      const require = createRequire(import.meta.url);
      // Playwright restricts subpath imports via package.json `exports`,
      // so `require.resolve("playwright/cli.js")` throws
      // ERR_PACKAGE_PATH_NOT_EXPORTED. package.json itself is always
      // exported by convention -- resolve that, then walk to cli.js.
      const pkgPath = require.resolve("playwright/package.json");
      playwrightCli = join(dirname(pkgPath), "cli.js");
    } catch (e) {
      reject(new Error(`could not resolve playwright cli: ${e.message}`));
      return;
    }
    const child = spawn(
      process.execPath,
