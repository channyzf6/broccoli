...
async function probeCodexDir(cwd) {
  ...
  const now = Date.now();
  let mostRecent = 0;
  // Same CANDIDATE_DAY_COUNT (3) as codex.mjs._resolvePath, anchored on
  // wall-clock now (vs sessionStart inside the adapter). The two are
  // close in practice; this is intentionally a slightly different
  // anchor since detectHost runs at proxy startup.
  for (let i = 0; i < 3; i++) {
