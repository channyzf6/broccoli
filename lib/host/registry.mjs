...
// line carries the cwd in its session_meta payload. We scan today's and
// yesterday's UTC date dirs (covers sessions that started near midnight),
// read the first ~2KB of each rollout-*.jsonl, and match cwd. Returns
// {mtime} of the most-recent matching file or null.
async function probeCodexDir(cwd) {
  ...
  for (const d of [today, yesterday]) {
    ...
    for (const f of entries) {
      ...
      // Read first 2KB only — first line is always session_meta.
      let metaCwd = null;
      try {
        ...
        try {
          ...
          if (bytesRead) {
            ...
            if (obj?.type === "session_meta" && obj.payload?.cwd) {
              metaCwd = obj.payload.cwd;
            }
          }
        } finally { await fh.close(); }
      } catch { continue; }
      if (!metaCwd) continue;
      const mc = String(metaCwd).toLowerCase().replace(/\\/g, "/").replace(/\/+$/, "");
      if (mc !== want) continue;
      try {
