...
// Probe Copilot's session-state tree. Each session lives at
// ~/.copilot/session-state/<uuid>/ with a workspace.yaml whose `cwd`
// field is the canonical match key. Returns {mtime} of the most-recent
// matching dir (events.jsonl mtime, falling back to the dir's own
// mtime). Honors COPILOT_HOME override.
async function probeCopilotDir(cwd) {
  const homeRoot = process.env.USERPROFILE || process.env.HOME;
  const home = process.env.COPILOT_HOME
    || (homeRoot ? path.join(homeRoot, ".copilot") : null);
  if (!home) return null;
  ...
  for (const e of entries) {
    ...
    if (!st.isDirectory()) continue;
    let yaml;
    try { yaml = await fsp.readFile(path.join(dir, "workspace.yaml"), "utf8"); }
    catch { continue; }
    const m = /^cwd:\s*(?:"([^"]*)"|(.+?))\s*$/m.exec(yaml);
    if (!m) continue;
    const dirCwd = (m[1] !== undefined ? m[1] : m[2]).trim();
    const norm = dirCwd.toLowerCase().replace(/\\/g, "/").replace(/\/+$/, "");
    if (norm !== want) continue;
    let mtime = st.mtimeMs;
    try {
      const evSt = await fsp.stat(path.join(dir, "events.jsonl"));
      if (evSt.mtimeMs > mtime) mtime = evSt.mtimeMs;
 ...
 *      (Claude > Gemini > Codex > Copilot) for backwards-compat.
...
export async function detectHost({ cwd } = {}) {
  ...
  if (declared === HOST.COPILOT) return HOST.COPILOT;
  if (!cwd) return HOST.CLAUDE;

  const [claudeHit, geminiHit, codexHit, copilotHit] = await Promise.all([
    probeClaudeDir(cwd),
    probeGeminiDir(cwd),
    probeCodexDir(cwd),
    probeCopilotDir(cwd),
  ...
  const hits = [
    ...
    { host: HOST.COPILOT, hit: copilotHit },
  ].filter((x) => x.hit);
...
export function makeAdapter({ host, cwd, sessionStart, pid }) {
  ...
  if (host === HOST.COPILOT) return new CopilotAdapter({ cwd, sessionStart, pid });
