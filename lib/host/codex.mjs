...
// 8KB peek for the first-line session_meta. session_meta can carry
// instructions / git context / cli_version metadata; modern Codex
// builds emit ~1-3KB but 8KB is cheap insurance against a future
// bump pushing us past the previous 2KB ceiling.
const META_PEEK_BYTES = 8 * 1024;
  ...
  async discoverName() {
    ...
    try {
      ...
      for (const line of text.slice(0, lastNl).split("\n")) {
        ...
        const raw = String(obj?.payload?.payload?.thread_name ?? "").trim();
