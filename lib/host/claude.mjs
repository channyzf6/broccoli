...
export class ClaudeAdapter extends HostAdapter {
  ...
  async _resolvePath() {
    ...
    let eligible = []; // every JSONL that passed the mtime filter
    for (const f of entries) {
      ...
      eligible.push(fp);
      // For OUR JSONL one of (first-line, last-line) ts is close to
      // sessionStart: NEW sessions match on first-line (the
      // session-init lines are written ≈ at sessionStart), RESUMED
      // sessions match on last-line (post-resume writes are recent).
      // Sibling JSONLs anchor on THEIR own sessionStart, so neither
      // end is close to ours unless they happen to be ≈ concurrent.
      const lastTs = await this._readLastLineTimestamp(fp, st.size);
      const firstTs = await this._readFirstLineTimestamp(fp, st.size);
      let delta = Infinity;
      if (lastTs) delta = Math.min(delta, Math.abs(lastTs - sessionStartMs));
      if (firstTs) delta = Math.min(delta, Math.abs(firstTs - sessionStartMs));
    ...
    // Single-candidate fallback. The 30s window exists to disambiguate
    // SIBLING JSONLs from concurrent / resumed sessions. With exactly
    // one eligible candidate there's nothing to disambiguate — and the
    // window legitimately closes for lazy-mode proxies whose first MCP
    // tool call (which triggers the first _resolvePath) happens many
    // minutes after sessionStart. Without this fallback, /rename never
    // propagates for those sessions.
    if (eligible.length === 1) {
      this._resolvedPath = eligible[0];
      return eligible[0];
    }
    return null;
  }

  // Read the first 16 KB of `fp` and return the earliest parseable
  // line's timestamp (epoch ms), or null on read/parse failure. CC
  // writes several timestamp-less init lines at the top
  // (custom-title, agent-name, permission-mode, ...); the first line
  // with a timestamp lands around line 7 in practice. 16 KB safely
  // covers that under all observed session-init sizes. For OUR
  // (newly-started) session this ts ≈ sessionStart, so it provides a
  // strong "this JSONL belongs to us" signal that survives long
  // sessions where the last-line ts has drifted past the 30s window.
  async _readFirstLineTimestamp(fp, size) {
    if (!size) return null;
    const READ = 16384;
    let fh;
