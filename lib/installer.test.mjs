...
//
// Also covers the copilot mcp-config.json patcher — copilot's `mcp add`
// is interactive-only, so the installer writes the JSON config directly.

import { test } from "node:test";
import assert from "node:assert/strict";
import { patchCodexEnvVars, patchCopilotConfig } from "./installer.mjs";
...
test("patchCopilotConfig: empty input creates a fresh document with our entry", () => {
  const out = patchCopilotConfig("", { command: "npx", args: ["-y", "sessions-dashboard@0.6.0"] });
  const doc = JSON.parse(out);
  assert.deepEqual(Object.keys(doc.mcpServers), ["sessions-dashboard"]);
  const e = doc.mcpServers["sessions-dashboard"];
  assert.equal(e.type, "local");
  assert.equal(e.command, "npx");
  assert.deepEqual(e.args, ["-y", "sessions-dashboard@0.6.0"]);
  assert.equal(e.env.SESSIONS_DASHBOARD_HOST, "copilot");
  assert.equal(e.env.SESSIONS_DASHBOARD_AUTOSTART, "1");
  assert.deepEqual(e.tools, ["*"]);
});

test("patchCopilotConfig: preserves other mcpServers entries", () => {
  const original = JSON.stringify({
    mcpServers: {
      "other-server": { command: "node", args: ["/path/to/other.mjs"] },
    },
  }, null, 2);
  const out = patchCopilotConfig(original, { command: "npx", args: ["-y", "sessions-dashboard@0.6.0"] });
  const doc = JSON.parse(out);
  assert.equal(doc.mcpServers["other-server"].command, "node", "sibling entry must survive");
  assert.equal(doc.mcpServers["sessions-dashboard"].command, "npx");
});

test("patchCopilotConfig: upgrades version pin when re-run", () => {
  const first = patchCopilotConfig("", { command: "npx", args: ["-y", "sessions-dashboard@0.5.0"] });
  const second = patchCopilotConfig(first, { command: "npx", args: ["-y", "sessions-dashboard@0.6.0"] });
  const doc = JSON.parse(second);
  assert.deepEqual(doc.mcpServers["sessions-dashboard"].args, ["-y", "sessions-dashboard@0.6.0"]);
});

test("patchCopilotConfig: bad JSON gets replaced wholesale", () => {
  const out = patchCopilotConfig("{not json", { command: "npx", args: ["-y", "sessions-dashboard@0.6.0"] });
  const doc = JSON.parse(out);
  assert.equal(doc.mcpServers["sessions-dashboard"].command, "npx");
});

test("patchCopilotConfig: preserves non-mcpServers top-level keys", () => {
  const original = JSON.stringify({
  ...
  const out = patchCopilotConfig(original, { command: "npx", args: ["-y", "sessions-dashboard@0.6.0"] });
  const doc = JSON.parse(out);
  assert.equal(doc.someUserSetting, "preserved");
  assert.ok(doc.mcpServers["sessions-dashboard"]);
});

test("patchCopilotConfig: idempotent — second run produces identical output", () => {
  const args = { command: "npx", args: ["-y", "sessions-dashboard@0.6.0"] };
  const once = patchCopilotConfig("", args);
  const twice = patchCopilotConfig(once, args);
