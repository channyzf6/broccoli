...
async function raiseWezTerm() {
  ...
  // Step 2: raise. We always run the full sequence even if AppActivate
  // claims success — testing on Win11 22H2 showed AppActivate returns
  // True without actually keeping the window forward when called from
  // a daemon-spawned PowerShell (no foreground privilege). The
  // sequence:
  ...
  //   - BringWindowToTop + ShowWindowAsync(SW_RESTORE) handle the
  //     minimized case and reorder.
  ...
  const raiseScript = [
    ...
    "[W]::keybd_event(0x12, 0, 2, [IntPtr]::Zero) | Out-Null",          // Alt up (KEYEVENTF_KEYUP=2)
...
// Spawn `wezterm cli activate-pane --pane-id <id>`. paneId must be
// a numeric string (caller validates via /^\d+$/). socket, when
// non-null, is passed as WEZTERM_UNIX_SOCKET in the spawned env so
// the CLI talks to the GUI's mux instead of auto-spawning a separate
// headless one (the env-var name is misleading on Windows — it's a
// named-pipe path, but WezTerm uses the same env var on both
// platforms).
