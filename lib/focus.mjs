...
async function tmuxClientTty() {
  const r = await runCmd("/usr/bin/env", ["tmux", "list-clients", "-F", "#{client_tty}"], { timeoutMs: PS_TIMEOUT_MS });
  if (r.code !== 0) return null;
...
async function focusViaTmux(tmuxPane, outerTty, termProgram) {
  // Step 1: switch the active tmux pane.
  if (!/^%\d+$/.test(String(tmuxPane))) {
    return { ok: false, error: "invalid tmux pane id: " + tmuxPane };
  }
  const switchR = await runCmd("/usr/bin/env", ["tmux", "switch-client", "-t", tmuxPane], { timeoutMs: PS_TIMEOUT_MS });
  if (switchR.code !== 0) {
...
export async function focusSession(session) {
  ...
  if (env.TMUX && env.TMUX_PANE) {
    // Detect nested tmux (TMUX with multiple path segments is rare; most
    // tmux installs write a single path). Day-1: don't try to guess.
    const outerTty = await tmuxClientTty();
    return await focusViaTmux(env.TMUX_PANE, outerTty, termProgram);
  }
