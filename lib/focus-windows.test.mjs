...
test("real activatePane: passes argv shape and WEZTERM_UNIX_SOCKET env", async () => {
  if (process.platform !== "win32") return;
  let captured = null;
  await withStubs({
    ...
    assert.deepEqual(captured.args, ["cli", "activate-pane", "--pane-id", "42"]);
...
test("real raiseWezTerm: valid pid,hwnd → calls raise script and returns ok", async () => {
  ...
  await withStubs({
    ...
    assert.match(calls[1].script, /SetForegroundWindow/, "raise script must call SetForegroundWindow");
    assert.match(calls[1].script, /keybd_event/, "raise script must use the Alt-key trick to lift foreground-lock");
  });
});
